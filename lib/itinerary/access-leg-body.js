...
export default class AccessLegBody extends Component {
  ...
  render () {
    ...
    if (leg.mode === 'CAR' && leg.hailedCar) {
      return <div><TNCWrapper componentClass={TNCLeg} leg={leg} legMode={legMode} /></div>
    }

    return (
      <div className='leg-body'>
        <AccessLegSummary leg={leg} legMode={legMode} />
...
class TNCLeg extends Component {
  render () {
    const { eta, rideEstimate, leg, legMode, LYFT_CLIENT_ID, UBER_CLIENT_ID } = this.props
    const universalLinks = {
      'UBER': `https://m.uber.com/${otpUtils.ui.isMobile() ? 'ul/' : ''}?client_id=${UBER_CLIENT_ID}&action=setPickup&pickup[latitude]=${leg.from.lat}&pickup[longitude]=${leg.from.lon}&pickup[nickname]=${encodeURI(leg.from.name)}&dropoff[latitude]=${leg.to.lat}&dropoff[longitude]=${leg.to.lon}&dropoff[nickname]=${encodeURI(leg.to.name)}`,
    ...
    return (
      <div>
        <div className='place-subheader'>
          Wait {Math.round(eta.estimatedSeconds / 60)} minutes for {eta.displayName} pickup
        </div>
        ...
        <div className='leg-body'>
          ...
          <a
            ...
            href={universalLinks[eta.company]}
