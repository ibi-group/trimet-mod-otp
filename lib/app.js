...
class TrimetWebapp extends Component {
  ...
  _updateMobileScreen (props) {
    ...
    // determine new screen to transition to, if any
    let newScreen = null

    // do not update screen if still setting date/time
    if (currentScreen === MobileScreens.SET_DATETIME) return

    // do not update screen if still setting 'to' location on initial screen
    if (currentScreen === MobileScreens.INITIAL_SET_LOCATION && props.query && !props.query.to) return
  ...
  render () {
        ...
        <Grid>
          <Row className='main-row'>
            <Col sm={6} md={4} className='sidebar'>
              <SearchForm showDateTime showSettings showPlanTripButton />
              <ErrorMessage />
              <div className='desktop-narrative-container'>
                {query.type === 'ITINERARY' && <NarrativeItineraries itineraryClass={TriMetItinerary} />}
                {query.type === 'PROFILE' && <NarrativeProfileOptions itineraryClass={TriMetItinerary} customIcons={customIcons} />}
              </div>
            </Col>
    ...
    switch (this.state.mobileScreen) {
      ...
      case MobileScreens.SET_OPTIONS:
        ...
        mobileTopPanel = <SearchForm mobile
...
class SearchForm extends Component {
  ...
  render () {
    const { onLocationFocused, showDateTime, showSettings } = this.props
...
const mapStateToProps = (state, ownProps) => {
  const activeSearchObj = state.otp.searches.length > 0 ? state.otp.searches[state.otp.activeSearch] : null
  return {
    browser: state.browser,
    currentPosition: state.otp.location.currentPosition,
    query: state.otp.currentQuery,
    activeSearch: state.otp.activeSearch,
    resultCount: activeSearchObj
      ? activeSearchObj.type === 'ITINERARY'
          ? (activeSearchObj.planResponse.plan ? activeSearchObj.planResponse.plan.itineraries.length : 0)
          : activeSearchObj.profileResponse.otp.profile.length
      : null
  }
}
