...
class TrimetWebapp extends Component {
  ...
  componentDidMount () {
    ...
    // TODO: move this check to otp-react-redux
  ...
  componentDidUpdate () {
    if (this.mapResized) this.refs.map.getWrappedInstance().resized()
  }
  ...
  _updateMobileScreen (props) {
    ...
    // do not change if in results view / summary. (Only way to leave that screen is by pressing "Edit" button)
    if (currentScreen === MobileScreens.RESULTS_SUMMARY || currentScreen === MobileScreens.RESULTS_DETAILS) return
    ...
    if (newScreen !== currentScreen) { // if we're changing screens..
      // trigger a map resize in componentDidUpdate for map-active screens
      if (newScreen === MobileScreens.RESULTS_SUMMARY || newScreen === MobileScreens.SET_OPTIONS) {
        this.mapResized = true
      }
  ...
  render () {
    const { query, realtimeEffects, searchPending, useRealtime } = this.props

    /** shared components **/
    const map = (
      <ToggleMap>
        <BaseMap ref="map" toggleName={<span><i className='fa fa-map' /> Map View</span>}>
        ...
        <StylizedMap toggleName={<span><i className='fa fa-random' /> Network View</span>} />
      </ToggleMap>
    )
    ...
    switch (this.state.mobileScreen) {
      ...
      case MobileScreens.RESULTS_SUMMARY:
        ...
        headerAction = realtimeEffects.isAffectedByRealtimeData &&
          <RealtimeAnnotation
      ...
      case MobileScreens.RESULTS_DETAILS:
        ...
        mobileTopPanel = locationsSummaryPanel
        /*mobileMapStyle.top = -window.innerHeight
        mobileMapStyle.height = window.innerHeight - 220*/
        showMap = false
...
class SearchForm extends Component {
  ...
  render () {
    ...
    const fromText = 'start location'
    const toText = 'destination'
    const actionText = mobile ? 'long press' : 'right-click'
    const fromLocationField = <LocationField
      type='from'
      label={`Enter ${fromText} or ${actionText} on map...`}
      onClick={() => {
...
const mapStateToProps = (state, ownProps) => {
  ...
  const realtimeEffects = otpUtils.state.getRealtimeEffects(state.otp)
  return {
...
const mapDispatchToProps = (dispatch, ownProps) => {
  return {
    ...
    setUseRealtimeResponse: ({useRealtime}) => dispatch(setUseRealtimeResponse({useRealtime})),
