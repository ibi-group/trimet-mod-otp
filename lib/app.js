...
const mapStateToProps = (state, ownProps) => {
  ...
  const latestSearch = state.otp.searches.length > 0 ? state.otp.searches[state.otp.searches.length - 1] : null

  return {
    browser: state.browser,
    currentPosition: state.otp.location.currentPosition,
    query: state.otp.currentQuery,
    queryChanged: !deepEqual(latestSearch ? latestSearch.query : null, state.otp.currentQuery),
    activeSearch: state.otp.activeSearch,
    searchPending: latestSearch && latestSearch.pending,
