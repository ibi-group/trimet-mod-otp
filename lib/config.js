...
import { BiketownIcon, LegIcon, StandardModeIcon, StandardStreetcar } from '@opentripplanner/icons'

// If you need to build a custom ModeIcon component,
// below is an example using ClassicModeIcon (icons in "classic" OTP-react-redux) as base.
// (You will need to include your own icon files as well.)
//
...
//   if (mode === 'RAIL') return <MyRailIcon {...props} />
//   if (mode === 'TRANSIT') return <MyTransitIcon {...props} />
//   // etc.
//
//   // For the remaining icons, just fall back to ClassicModeIcon.
...
const TriMetLegIcon = ({ leg, ...props }) => {
  ...
  if (leg.rentedBike) {
    ...
    return <StandardModeIcon mode='BICYCLE' {...props} />;
  }
