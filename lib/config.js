...
import { BatchRoutingPanel, CallTakerPanel } from 'otp-react-redux'
 ...
 * - getMainPanel (React component(*), optional)
...
export function configure (otpConfig) {
  ...
  return {
    ...
    /**
     * This function will return a React Element for the main side panel (where
     * the form elements and itinerary narratives are). This function is run
     * once on application startup.
     */
    getMainPanel: () => {
      if (otpConfig.datastoreUrl) {
        // If Calltaker module is enabled, use CallTaker panel.
        return (
          <CallTakerPanel
            LegIcon={LegIcon}
            ModeIcon={ModeIcon} />
          )
      } else if (otpConfig.routingTypes.find(t => t.key === 'BATCH')) {
        console.log('Using batch routing panel')
        // If Batch routing is enabled, use batch routing panel.
        return <BatchRoutingPanel LegIcon={LegIcon} ModeIcon={ModeIcon} />
      }
      // Return null be default (which will prompt the use of the default panel)
      return <BatchRoutingPanel LegIcon={LegIcon} ModeIcon={ModeIcon} />
    },
    LegIcon,
    ModeIcon
