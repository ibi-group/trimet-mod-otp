      ...
      <div className='itin-summary'>
        ...
        <div className='routes'>
          {itinerary.legs.filter(leg => {
            return leg.transitLeg || (leg.mode === 'CAR' && companies)
          }).map((leg, k) => {
...
class ItineraryBody extends Component {
  ...
  render () {
    ...
    return (
      <div className='itin-body'>{rows}</div>
    )
  }
}
    ...
    return (
      <div className='leg-body'>
        <div onClick={() => this._onHeaderClick()} className='header'>
          ...
          {toSentenceCase(leg.mode)} {otpUtils.time.formatDuration(leg.duration)} {notation}
